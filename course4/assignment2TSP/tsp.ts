...
import { getDistanceMatrix, getEuclideanDistance } from "./utils";
...
export const getTspFromFile = async (filename: string) => {
  ...
  return getTSP(cities, distanceMatrix);
};
...
export const optimisedGetTspFromFile = async (filename: string) => {
  ...
  const citiesLeft = cities.slice(0, 13);
  const citiesRight = cities.slice(11);
  const tsp1 = getTSP(citiesLeft, getDistanceMatrix(citiesLeft));
  const tsp2 = getTSP(citiesRight, getDistanceMatrix(citiesRight));
  return tsp1 + tsp2 - 2 * getEuclideanDistance(cities[11], cities[12]);
};

export const getTSP = (cities: city[], distanceMatrix: number[][]) => {
...
export const getTSPOptimised = (cities: city[]) => {
  const distanceMatrix = getDistanceMatrix(cities);
  const numberCities = cities.length;
  const allPossibleSubsets = generateBinaryNumbers(numberCities);
  // filter those that include city 1 and convert to integer
