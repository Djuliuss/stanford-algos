...
console.info(
  multiply(x.split("").map(Number), y.split("").map(Number))
    .map(String)
    .join("")
);
...
function arrayAdd(e, f) {
  ...
  for (var i = 1; i <= addend.length; i++) {
    singleDigitSum =
      augend[augend.length - i] + addend[addend.length - i] + carry;
...
function arraySubtract(g, h) {
  ...
  if (borrow == 1) {
    return arraySubtract(g.slice(0, g.length - h.length), [1]).concat(
      diffArray
    );
  } else {
...
function recursMultiply(m, n) {
  ...
  var splitLength = Math.pow(
    2,
    Math.ceil(Math.log2(Math.max(m.length, n.length) / 2))
  );
  ...
  var ac = multiply(a, c);
  var bd = multiply(b, d);
  var middleTerm = arraySubtract(
    arraySubtract(multiply(arrayAdd(a, b), arrayAdd(c, d)), ac),
    bd
  );
